...
//				}
	...
	public int singleDimensionSampling(int dimIndex) throws Exception{
		...
		for(int i:subsegindexes){
			...
			if(ni==1){
				logLikelihood[i]=Manager.getLogLikelihood(validating)+Math.log(alpha/(n-1+alpha));
			}else{
				logLikelihood[i]=Manager.getLogLikelihood(validating)+Math.log((ni-1)/(n-1+alpha));
			}
