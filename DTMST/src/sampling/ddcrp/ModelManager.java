...
//			}
//		}
		logLikelihood+=-(Math.pow(validating.instance(dimIndex).classValue()-mean,2)/var)-0.5*Math.log(2*Math.PI*var);
		
//		Segmentation segment=segmentations.get(segIndex);
//			logLikelihood+=-segment.cells.size()*(log_pi2+Math.log(segment.VAR)-1);
